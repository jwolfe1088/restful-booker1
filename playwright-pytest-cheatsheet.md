...
Pythonfrom playwright.sync_api import sync_playwright, expect
...
Python# conftest.py (place in tests/ or root)
import pytest
...
Python# tests/test_api_bookings.py
import pytest
...
Python# Get auth token first (reusable fixture)
@pytest.fixture(scope="session")
...
Pythondef test_get_bookings_with_query(api_request):
...
Python@pytest.mark.parametrize("username, password, expected_status", [
